...
function makeRequest(
	...
	return new Request("https://example.com/api/tiktok-shop/webhooks", {
		method: "POST",
...
describe("createStoreEndpoints — tiktok-shop", () => {
	it("returns an endpoint map with the /tiktok-shop/webhooks path", () => {
		const endpoints = createStoreEndpoints();
		expect(endpoints).toHaveProperty("/tiktok-shop/webhooks");
		expect(endpoints["/tiktok-shop/webhooks"]).toBeDefined();
	});
