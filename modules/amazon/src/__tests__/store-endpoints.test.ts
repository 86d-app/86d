...
function makeRequest(
	...
	return new Request("https://example.com/api/amazon/webhooks", {
		method: "POST",
...
describe("createStoreEndpoints — amazon", () => {
	it("returns an endpoint map with the /amazon/webhooks path", () => {
		const endpoints = createStoreEndpoints();
		expect(endpoints).toHaveProperty("/amazon/webhooks");
		expect(endpoints["/amazon/webhooks"]).toBeDefined();
	});
	...
	it("endpoint returns 400 for invalid JSON body", async () => {
		...
		const res = await callEndpoint(endpoints, "/amazon/webhooks", req);
