...
function makeRequest(
	...
	return new Request("https://example.com/api/facebook-shop/webhooks", {
		method: "POST",
...
describe("createStoreEndpoints — facebook-shop", () => {
	it("returns an endpoint map with the /facebook-shop/webhooks path", () => {
		const endpoints = createStoreEndpoints();
		expect(endpoints).toHaveProperty("/facebook-shop/webhooks");
		expect(endpoints["/facebook-shop/webhooks"]).toBeDefined();
	});
	...
	it("endpoint returns 400 for invalid JSON body", async () => {
		...
		const res = await callEndpoint(endpoints, "/facebook-shop/webhooks", req);
